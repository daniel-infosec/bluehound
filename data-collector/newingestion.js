...
//https://stackoverflow.com/a/5801971
module.exports = {
    buildGroupJsonNew: function (chunk) {
        let queries = {};
        queries.properties = {};
        ...
        for (let group of chunk) {
            ...
            let grouped = lodash.groupBy(members || [], 'MemberType');
    ...
    buildComputerJsonNew: function (chunk) {
        let queries = {};
        queries.properties = {};
        queries.properties.statement =
        ...
        for (let computer of chunk) {
            ...
            let grouped = lodash.groupBy(allowedToAct || [], 'MemberType');
            for (let group in grouped) {
                format[0] = group;
            ...
            grouped = lodash.groupBy(localAdmins || [], 'MemberType');
            for (let group in grouped) {
                format[0] = group;
                format[2] = 'AdminTo';
                props = grouped[group].map(group => {
            ...
            grouped = lodash.groupBy(rdp || [], 'MemberType');
            for (let group in grouped) {
                format[0] = group;
                format[2] = 'CanRDP';
                props = grouped[group].map(group => {
            ...
            grouped = lodash.groupBy(dcom || [], 'MemberType');
            for (let group in grouped) {
                format[0] = group;
            ...
            grouped = lodash.groupBy(psremote || [], 'MemberType');
            for (let group in grouped) {
                format[0] = group;
    ...
    buildUserJsonNew: function (chunk) {
        let queries = {};
        queries.properties = {
            statement:
        ...
        for (let user of chunk) {
            ...
            let grouped = lodash.groupBy(sidHistory, 'MemberType');
    ...
    buildGpoJsonNew: function (chunk) {
        let queries = {};
        queries.properties = {
            statement:
    ...
    buildOuJsonNew: function (chunk) {
        ...
        for (let ou of chunk) {
            ...
            let grouped = lodash.groupBy(admins, 'MemberType');
            for (let x in grouped) {
                format[0] = x;
                format[2] = 'AdminTo';
            ...
            grouped = lodash.groupBy(psRemoteUsers, 'MemberType');
            for (let x in grouped) {
                format[0] = x;
                format[2] = 'CanPSRemote';
                let flattened = computers.flatMap(computer => {
            ...
            grouped = lodash.groupBy(dcomUsers, 'MemberType');
            for (let x in grouped) {
                format[0] = x;
                format[2] = 'ExecuteDCOM';
                let flattened = computers.flatMap(computer => {
            ...
            grouped = lodash.groupBy(rdpUsers, 'MemberType');
    ...
    buildDomainJsonNew: function (chunk) {
        let queries = {};
        queries.properties = {
            statement:
...
function processAceArrayNew(aces, objectid, objecttype, queries) {
    ...
    var grouped = lodash.groupBy(convertedAces, 'right');
    let format = [
        '',
        objecttype,
        '',
    ...
    for (let right in grouped) {
        let innerGrouped = lodash.groupBy(grouped[right], 'pType');
...
function processSPNTargetArrayNew(spns, objectid, queries) {
    ...
    let grouped = lodash.groupBy(spns, 'Service');
    for (let group in grouped) {
        format[2] = group;
